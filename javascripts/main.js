...
async function init() {
    ...
    heartbeat = new Audio('/static/sounds/heartbeat.mp3');
...
function player_collide(step_dir, check) {
    ...
    for (let i = 0; i < maze.black_hole_group.children.length; i++) {
        ...
        if (player_box_x.intersectsBox(black_hole.geometry.boundingBox)) {
            if(MODE_SOUND){
                let sound = new Audio('/static/sounds/black_hole.mp3');
...
function play_coin_sound(){
    if (!MODE_SOUND)
        return
    const sound = new Audio('/static/sounds/coin.mp3');
    sound.volume = 0.05;
    sound.play();
}
...
function game_end(success){
    ...
    if(success){
        if (MODE_SOUND) {
            const sound = new Audio('/static/sounds/pac_man_victory.mp3');
            sound.volume = 0.05;
            sound.play();
        }
        ...
        if (MODE_SOUND) {
            const sound = new Audio('/static/sounds/pac_man_death.mp3');
            sound.volume = 0.05;
            sound.play();
        }
